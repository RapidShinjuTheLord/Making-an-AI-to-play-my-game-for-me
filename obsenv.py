...
class StayingAlive(Env):
    def __init__(self, will_render = False):
        ...
        self.observation_space = spaces.Box(low = -1000,high = 1000,shape = self.observation_shape, dtype=np.float64)
    ...
    def draw_elements_on_canvas(self):
        ...
        self.canvas[0] = self.player.position[0]
        self.canvas[1] = self.player.position[1]
        ...
        for danger in self.board.obstacles[::-1]:
            ...
            if danger.index == 'laser':
                if danger.axis:
                    pass#self.canvas[0][int(danger.pos/4)][1] = danger.time/danger.initial_time
                elif not danger.axis:
                    pass#self.canvas[int(danger.pos/4)][0][1] = danger.time/danger.initial_time
            elif danger.index == 'plasma':
                #self.canvas[int(danger.pos[0]/4)][int(danger.pos[1]/4)][2] = 1
                self.canvas[2] = danger.pos[0]
                self.canvas[3] = danger.pos[1]
                self.canvas[4] = danger.vector[0]
                self.canvas[5] = danger.vector[1]
            ...
            for danger in self.board.obstacles:
                if danger.index == 'plasma':
                    ...
                    if (self.player.position[0]+10 - danger.pos[0]) ** 2 + (self.player.position[1]+10-danger.pos[1]) ** 2 < (self.board.plasmawidth/2 + 12) ** 2:
                        ...
                        d1 = log(20 * (((self.player.position[0] + 10 - danger.pos[0]) ** 2 + (self.player.position[1] + 10 - danger.pos[1]) ** 2) ** 0.5) / (850) )
                        ...
                        self.reward  = d1 + 0.5*d2 + 0.5*d3 + 0.5*d4 + 0.5*d5

                        #self.reward = log(20 * d)
                        ...
                        if ((self.player.position[0]+10 - danger.pos[0]) ** 2 + (self.player.position[1]+10 - danger.pos[1]) ** 2) ** 0.5 < 200:
                            ...
                            #self.reward -= (200 - (((self.player.position[0] + 10 - danger.pos[0]) ** 2 + (self.player.position[1] + 10 - danger.pos[1]) ** 2) ** 0.5)) / 240
                            #self.reward = (((self.player.position[0] + 10 - danger.pos[0]) ** 2 + (self.player.position[1] + 10 - danger.pos[1]) ** 2) ** 0.5) / (850) - 0.5 * (((self.player.position[0] + 10 - 300)**2 + (self.player.position[1] + 10 - 300)**2)  ** 0.5) / 850
                    ...
                    #danger.pos[0] += danger.vector[0] / 1000
                    danger.pos[1] += danger.vector[1] / 200
                    #danger.pos[1] += danger.vector[1] / 1000


            self.gametime += 1

            #if self.player.position[0] < 20 or self.player.position[0] > 560:
            #    self.reward -= 4
            #if self.player.position[1] < 20 or self.player.position[1] > 560:
            #    self.reward -= 4


            if not self.epdone:
                self.draw_elements_on_canvas()
                self.reward += log(20 * 1)
            if self.epdone:
                pass
                self.reward += log(20 * 0.001)
