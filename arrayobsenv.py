...
class StayingAlive(Env):
    ...
    def draw_elements_on_canvas(self):
        ...
        self.canvas = np.zeros(self.observation_shape)
        self.canvas[0:][0][0] = [self.player.position[0], self.player.position[1], 0, 0]
        listofplasmas = []
        ...
        for idx, danger in enumerate(listofplasmas):
            try:
                self.canvas[0:][1][idx] = [danger.pos[0], danger.pos[1], danger.vector[0], danger.vector[1]]
            except:
                pass
    ...
    def step(self, action = 'none'):
        ...
        if not self.epdone:
            ...
            self.reward += 1 * d2 + 1 * d3 + 1 * d4 + 1 * d5
