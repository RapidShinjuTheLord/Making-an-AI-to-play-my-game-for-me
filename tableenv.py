...
class StayingAlive(Env):
    ...
    def draw_elements_on_canvas(self):
        ...
        for idx, danger in enumerate(listofflames):
            ...
            try:
                if danger.wall%2:
                    self.canvas[0:][1][idx] = [int(danger.getxy()[0]), int(danger.getxy()[1]), int(danger.time/danger.initial_time * 600), int(1)]
                    #column, channel, row
                else:
                    self.canvas[0:][1][idx] = [int(danger.getxy()[0]), int(danger.getxy()[1]), int(danger.time/danger.initial_time * 600), int(2)]
            except:
                pass
        for idx, danger in enumerate(listofbombs):
            try:
                self.canvas[0:][2][idx] = [int(danger.pos[0]), int(danger.pos[1]), int(danger.time/danger.initial_time * 600), 0]
            except:
                pass
        for idx, danger in enumerate(listoflasers):

            try:
                self.canvas[0:][3][idx] = [int(danger.getxy()[0]), int(danger.getxy()[1]), int(danger.time/danger.initial_time * 600), 0]
            except:
                pass

        for idx, danger in enumerate(listofplasmas):
            try:
                self.canvas[0:][4][idx] = [int(danger.pos[0]), int(danger.pos[1]), int(danger.vector[0]), int(danger.vector[1])]
            except:
                pass
    ...
    def distreward(self):
        ...
        for danger in self.board.obstacles[::-1]:
            if danger.index == 'flame':
                ...
                listofplasmas.append(danger)
        for danger in listofplasmas:
            d1 = log(20 * (((self.player.position[0] + 15 - danger.pos[0]) ** 2 + (
                        self.player.position[1] + 15 - danger.pos[1]) ** 2) ** 0.5) / (850))
            total += d1
        for danger in listofbombs:
            if danger.color == (255,0,0):
                d1 = log(20 * (((self.player.position[0] + 15 - danger.pos[0]) ** 2 + (
                        self.player.position[1] + 15 - danger.pos[1]) ** 2) ** 0.5) / (850))
                total += d1
            pass
        for danger in listoflasers:
            if danger.color == (255,0,0):
                if danger.axis%2:
                    distance = abs(danger.getxy()[0] - self.player.position[0] - 15)
                else:
                    distance = abs(danger.getxy()[1] - self.player.position[1] - 15)
                d1 = log(20 * ((distance/850)+0.01))
                total += d1
        for danger in listofflames:
            if danger.color == (255,0,0):
                if danger.wall%2:
                    positionofobs = (danger.getxy()[0] + self.board.flameheight/2, danger.getxy()[1] + self.board.flamewidth/2 )

                else:
                    positionofobs = (
                    danger.getxy()[0] + self.board.flamewidth / 2, danger.getxy()[1] + self.board.flameheight / 2)
                d1 = log(20 * (((self.player.position[0] + 15 - positionofobs[0]) ** 2 + (
                            self.player.position[1] + 15 - positionofobs[1]) ** 2) ** 0.5) / (850))
                total += d1

        d2 = log(20 * (self.player.position[0] + 10 - 0) / (850))

        d3 = log(20 * (self.player.position[1] + 10 - 0) / (850))

        d4 = log(20 * (600 - self.player.position[0] - 10) / (850))

        d5 = log(20 * (600 - self.player.position[1] - 10) / (850))

        total += 0.5 * d2 + 0.5 * d3 + 0.5 * d4 + 0.5 * d5
    ...
    def step(self, action = 'none'):
        ...
        if not self.epdone:
            ...
            self.reward = self.distreward()/20
